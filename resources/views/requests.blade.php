            ...
            <div class="col-8 col-md-8 col-lg-8">
                <div class="card">
                    <div class="card-header">
                      <h4>Request <a href="#" class="btn btn-primary"  title='New Request' data-toggle="modal" data-target="#newRequests"><i class='fas fa-plus'></i> New Request</a></h4>
                    </div>
                    <div class="card-body">
                      <div class="table-responsive">
                        <table class="table table-hover " id="employees-table" style="width:100%;">
                          ...
                          <tbody>
                           @foreach($request_inventory as $req)
                            <tr>
                                <td>TR-{{str_pad($req->id, 4, '0', STR_PAD_LEFT)}}</td>
                                <td>
                                    @foreach($req->inventory as $info)
                                    
                                    <small>{{$info->inventory->item_description}} - {{$info->request_qty}}  @if($req->status == "Approved")  -  @if($info->status == "For Deployment") <a href="#" title='{{$info->status}}' class="btn btn-icon btn-sm btn-info"><i class="fas fa-paper-plane"></i></a> @else <a href="#" title='{{$info->status}}'  class="btn btn-icon btn-sm btn-success"><i class="fas fa-user-check"></i></a> @endif</span> @endif</small> <br>
                                    @endforeach
                                </td>
                                <td><small>{!! nl2br(e($req->remarks)) !!}</small></td>
                                <td>
                                    <span class="badge @if($req->status == "Pending") badge-info  @elseif($req->status == "Approved") badge-success @else badge-danger  @endif">{{$req->status}}</span><br>
                                    <small>Approver : {{$req->approver->name}}</small>
                                </td>
                                <td>
                                    ...
                                    <a title='Cancel' href="#" class="btn btn-icon btn-danger" data-toggle="modal" data-target="#cancelRequest" onclick="cancel_request({{$req->id}})"><i class="fas fa-times"></i></a>
                                    @endif
                ...
                <div class="col-xl-12 col-lg-12">
                    <div class="card">
                        ...
                        <div class="row">
                            ...
                            <span class="font-weight-bold mb-0">{{$deployed_requests}}</span>
                            </div>
                            <div class="col-auto">
...
<script type="text/javascript">
    ...
    function view_request(id_def)
    {
        ...
        for (var i = 0; i < request_histories.length; i++) {
            ...
            var dataAssets = "<div class='row  border'>";
                dataAssets += "<div class='col-sm-3 border'><small>";    
                dataAssets += request_histories[i].user_info.name;      
                dataAssets += "</small></div>";    
                dataAssets += "<div class='col-sm-3 border'><small>";    
                dataAssets += nl2br(request_histories[i].remarks);      
                dataAssets += "</small></div>";    
                dataAssets += "<div class='col-sm-3 border'><small>";    
